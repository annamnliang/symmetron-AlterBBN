...
int main(int argc,char** argv){
  ...
  double massparam, muparam, vparam;
  double phi_init, phi_0;
  int failsafe;

  if(argc<5)
    {
        printf(" This program needs 4 parameters:\n"
        "   massparam   symmetron mass M parameter in log10(M/M_pl) \n"
        "   muparam     symmetron mass mu parameter in log10(mu/GeV) \n"
        "   vparam     	symmetron v parameter in log10(v/M_pl) \n"
        "   phi_init    dimensionless value of the field at T_init \n");
    ...
    {
        sscanf(argv[1],"%lf",&massparam);
        sscanf(argv[2],"%lf",&muparam);
        sscanf(argv[3],"%lf",&vparam);
        sscanf(argv[4],"%lf",&phi_init);
    ...
    paramrelic.mass = massparam;
    paramrelic.mu = muparam;
    paramrelic.vval = vparam;
    paramrelic.phi_init = phi_init;

    Init_vecs(&paramrelic);
